...
class WebViewRepositoryTests: XCTestCase {
    
    func testGivenAWebViewRepositoryWhenAddedToFavoriteThenIsCorrectlyAdded() {
        let stack = CoreDataStack(modelName: "Model", type: CoreDataStackType.test)
        let repository = WebViewRepository(stack: stack)
    ...
    func testGivenAWebVieweRepositoryWhenRemoveFromFavoriteThenIsCorrectlyRemoved() {
        let stack = CoreDataStack(modelName: "Model", type: CoreDataStackType.test)
        let repository = WebViewRepository(stack: stack)
        let article = VisibleArticle(category: "1",
                                     title: "1",
